...
class rcmail_action_settings_prefs_save extends rcmail_action
{
    ...
    public function run($args = [])
    {
        ...
        case 'customization':
            ...
            // save product name
            $productName = $_POST['_product_name'];
            if(! $productName) {
                $rcmail->output->show_message('Product name is required', 'error');
                $rcmail->overwrite_action('edit-prefs');
                return;
            }

            $oldProductName = $config['product_name'];
            if($oldProductName !== $productName) {
                $loadedConfig = file_get_contents(INSTALL_PATH . 'config/config.inc.php');
                $loadedConfig = str_replace($oldProductName, $productName, $loadedConfig);
                file_put_contents(INSTALL_PATH . 'config/config.inc.php', $loadedConfig);
            }

            // save logo
            $logo = $_POST['_logo'];
            $oldLogo = $config['skin_logo']['elastic:*'];
            $oldLogoExtension = substr($oldLogo, -3);
            if($logo !== $oldLogo) {
                if(! filter_var($logo, FILTER_VALIDATE_URL)) {
                    $rcmail->output->show_message('Logo URL isn\'t valid', 'error');
                    $rcmail->overwrite_action('edit-prefs');
                    return;
                }
                $extension = pathinfo(parse_url($logo, PHP_URL_PATH), PATHINFO_EXTENSION);
                if(! in_array($extension, ['jpg', 'png', 'svg'])) {
                    $rcmail->output->show_message('The logo must be in PNG, SVG or JPG format', 'error');
                    $rcmail->overwrite_action('edit-prefs');
                    return;
                }

                $imageString = file_get_contents($logo);
                file_put_contents(INSTALL_PATH . 'skins/elastic/images/logo.' . $extension, $imageString);

                if($extension !== $oldLogoExtension) {
                    $loadedConfig = file_get_contents(INSTALL_PATH . 'config/config.inc.php');
                    $loadedConfig = str_replace($oldLogo, '/images/logo.' . $extension, $loadedConfig);
                    file_put_contents(INSTALL_PATH . 'config/config.inc.php', $loadedConfig);
                }

                // update watermark
                $watermark = file_get_contents(INSTALL_PATH . 'skins/elastic/watermark.html');
                $oldString = 'url(images/logo.svg) center no-repeat #fff';
                if(strpos($watermark, 'logo.svg') === false) {
                    $oldString = 'url(images/logo.'.$oldLogoExtension.') center no-repeat #fff';
                }
                $watermark = str_replace($oldString, 'url(images/logo.'.$extension.') center no-repeat #fff', $watermark);
                file_put_contents(INSTALL_PATH . 'skins/elastic/watermark.html', $watermark);
            }

            // save favicon
            $favicon = $_POST['_favicon'];
            $oldFavicon = $config['skin_logo']['[favicon]'];
            $oldFaviconExtension = substr($oldLogo, -3);
            if($favicon !== $oldFavicon) {
                if(! filter_var($favicon, FILTER_VALIDATE_URL)) {
                    $rcmail->output->show_message('Favicon URL isn\'t valid', 'error');
                    $rcmail->overwrite_action('edit-prefs');
                    return;
                }
                $extension = pathinfo(parse_url($favicon, PHP_URL_PATH), PATHINFO_EXTENSION);
                if(! in_array($extension, ['ico'])) {
                    $rcmail->output->show_message('The favicon must be in ICO format', 'error');
                    $rcmail->overwrite_action('edit-prefs');
                    return;
                }

                $imageString = file_get_contents($favicon);
                file_put_contents(INSTALL_PATH . 'skins/elastic/images/favicon.' . $extension, $imageString);

                if($extension !== $oldFaviconExtension) {
                    $loadedConfig = file_get_contents(INSTALL_PATH . 'config/config.inc.php');
                    $loadedConfig = str_replace($oldFavicon, '/images/favicon.' . $extension, $loadedConfig);
                    file_put_contents(INSTALL_PATH . 'config/config.inc.php', $loadedConfig);
                }
            }
